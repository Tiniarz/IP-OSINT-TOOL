...
try:
    ...
    print(f'Modules of the python library required for IP OSINT TOOL are not installed, make sure you have correctly installed python and have launched the "Setup.py" file which will install all the necessary modules.')
    input(f'Error: {e}')
...
def Reset():
    if os_name == "Windows":
        file = ['python', os.path.join(tool_path, "IPOSINTTOOL.py")]
        subprocess.run(file)

    elif os_name == "Linux":
        file = ['python3', os.path.join(tool_path, "IPOSINTTOOL.py")]
        subprocess.run(file)
